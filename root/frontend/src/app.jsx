...
function App() {
  ...
  // Load daily history story whenever elder mode is selected
  useEffect(() => {
    if (userType !== "elder") return;

    async function loadDaily() {
      try {
        setDailyLoading(true);
        setDailyError(null);
        const res = await fetch("http://localhost:5050/api/daily-history");
        const data = await res.json();
        if (!res.ok || !data.summary) {
          throw new Error(data.error || "Failed to load daily story");
        }
        setDaily(data);
      } catch (err) {
        console.error("Daily story error:", err);
        setDailyError(
          "We couldn’t load today’s story, but you can still use the search above."
        );
      } finally {
        ...
        <section className="card elder-daily-card">
          ...
          {dailyLoading && (
            <p className="elder-muted">
              Fetching a short story from today in history…
            </p>
          )}
          {dailyError && <p className="elder-muted">{dailyError}</p>}
          {daily && !dailyLoading && !dailyError && (
            <>
              <h4 className="elder-daily-title">{daily.title}</h4>
              <p>{daily.summary}</p>
              <a href={daily.url} target="_blank" rel="noopener noreferrer">
